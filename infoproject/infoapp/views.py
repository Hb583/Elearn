...
def courses(request):
    ...
    for i in sdd:
        if i.Subject_title not in sd:
            ...
            count += 1
    return render(request, 'courses.html', {'sd': sd, 'count': count})
...
def register_tr(request):
    if request.method == 'POST':
        ...
        intro = request.POST.get('introduction_brief')
        photo = request.FILES['photo']
        fs = FileSystemStorage()
        fs.save(photo.name, photo)
        ...
        for i in reg1:
            if i.Email == email:
                ...
                return render(request, 'regtt.html')
        user_name = request.POST.get('username')
        for t in User.objects.all():
            if t.username == user_name:
                messages.success(request, 'username taken please take another')
                return render(request, 'regtt.html')
...
def reg_admin(request):
    if request.method == 'POST':
        ...
        fs = FileSystemStorage()
        fs.save(photo.name, photo)
        adm = Registration.objects.all()
        for j in adm:
            if j.Email == email:
                messages.success(request, 'Username already exists')
                return render(request, 'regadmin.html')
        user_name = request.POST.get('username')
        for z in User.objects.all():
            if z.username == user_name:
        ...
        j.Email = email
        j.Password = password
        j.Registration_date = y
        j.Qualification = 'Nil'
        j.Introduction_brief = 'Nil'
        j.image = photo
        j.Num_of_enrolled_students = 0
        j.Average_review_rating = 0
        j.Num_of_reviews = 0
        j.About_website = 'Nil'
        j.User_role = 'admin'
        j.user = user
...
def bnb(request):
    ...
    if request.method == 'POST':
        ...
        for t in m:
            if t.username == user_name:
                messages.success(request, 'Username taken. Please try another')
                return render(request, 'adm_prof.html', {'bb1': bb1, 'um': um})
...
def stu_buk_teacher(request):
    ...
    gg = Subject.objects.filter(Sub_reg=nm.id, Course_title=request.session['course'],
                                Subject_title=request.session['sub_n'])
    spp = Enrollment()
...
def acc_chapter(request, id, ikm, sub):
    ...
    dm = Subject.objects.filter(Subject_title=request.session['subb'], Course_title=request.session['courss'],
                                Sub_reg=nn.id)
    for i in dm:
...
def acc_chapter1(request):
    ...
    sz = Subject.objects.filter(Subject_title=request.session['subb'], Course_title=request.session['courss'],
                                Chapter_title=sz1, Sub_reg=request.session['tch_idd'])
    mj = Learning_progress.objects.filter(Student_name=mnm.First_name, Student_email=mnm.Email,
                                          Subject_name=request.session['subb'], Course_name=request.session['courss'],
                                          Course_chapter_content_name=sz1)
    ch_con_nnme = []
    ch_txt_con_id = []
    ch_txt_con = []
    ch_cont_nmbe = []
...
def m_m1(request):
    q = Registration.objects.get(id=request.session['logg'])
    tt = Messages.objects.filter(To_email=q.Email).exclude(Category='guest')
    return render(request, 'message3.html', {'tt': tt})


def del_msg_teacher(request, id):
    Messages.objects.get(id=id).delete()
    q = Registration.objects.get(id=request.session['logg'])
    tt = Messages.objects.filter(To_email=q.Email)
    messages.success(request, 'Message delete successfully')
    return render(request, 'message3.html', {'tt': tt})


def reply_msg_teacher(request, id):
    ta = Messages.objects.get(id=id)
    q = Registration.objects.get(id=request.session['logg'])
    tt = Messages.objects.filter(To_email=q.Email)
    if request.method == 'POST':
        f_email = request.POST.get('f_email')
        ...
        rmd = q.Last_name
        ta1 = Messages()
        ta1.Name = str(rm) + ' ' + str(rmd)
        ta1.Category = q.User_role
        ta1.From_email = to_email
        ta1.To_email = f_email
        ta1.Message_content = msg_cont
        ta1.save()
        messages.success(request, 'Message reply successful')
        return render(request, 'message3.html', {'tt': tt})
    return render(request, 'reply_msg_teacher.html', {'ta': ta})


def sent_msg_teacher(request):
    kkt = Registration.objects.all()
    pt = Registration.objects.get(id=request.session['logg'])
    bbt = Messages.objects.filter(To_email=pt.Email)
    if request.method == 'POST':
        to_em = request.POST.get('to_em')
        ddp = str(to_em)
        gg = ddp.split()
        ...
        nm.save()
        messages.success(request, 'Message sent successfully')
        return render(request, 'message3.html', {'bbt': bbt})
    return render(request, 'sent_msg_teacher.html', {'kkt': kkt})


def m_m3(request):
    q = Registration.objects.get(id=request.session['logg'])
    tt = Messages.objects.filter(To_email=q.Email).exclude(Category='guest')
    return render(request, 'message4.html', {'tt': tt})
...
def reply_msg_admin(request, id):
    ...
    if request.method == 'POST':
        ...
        messages.success(request, 'Message reply successful')
        return render(request, 'message4.html', {'tt': tt})
    return render(request, 'reply_msg_admin.html', {'ta': ta})
...
def sent_msg_admin(request):
    ...
    if request.method == 'POST':
        ...
        nm.save()
        messages.success(request, 'Message sent successfully')
        return render(request, 'message4.html', {'bbt': bbt})
    return render(request, 'sent_msg_admin.html', {'kkt': kkt})
